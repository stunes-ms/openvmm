...
//! SHA-384 implementation using the Windows BCrypt API.
...
pub struct Sha384(BCRYPT_HASH_HANDLE);

impl Sha384 {
    pub fn new() -> Self {
        let mut handle = BCRYPT_HASH_HANDLE::default();
        // SAFETY: the SHA-384 pseudo-handle is a valid process-wide algorithm
        // handle; no secret and no caller-allocated object buffer are needed.
        // All inputs are known-good, so the call must succeed.
        unsafe {
            BCryptCreateHash(BCRYPT_SHA384_ALG_HANDLE, &mut handle, None, None, 0).unwrap();
        }
        Self(handle)
    }
    ...
    pub fn update(&mut self, data: &[u8]) {
        ...
        // The call cannot fail for a valid SHA-384 hash handle.
        unsafe {
    ...
    pub fn finish(self) -> [u8; 48] {
        let mut out = [0u8; 48];
        // SAFETY: handle is valid and owned; output buffer matches the SHA-384
